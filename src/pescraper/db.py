...
# complete/needs_review may re-enter in_progress (a stale-firm re-check, per the
# 90-day re-queue requirement) but never jump directly back to pending.
ALLOWED_TRANSITIONS: dict[str, frozenset[str]] = {
    FirmStatus.PENDING.value: frozenset({FirmStatus.IN_PROGRESS.value}),
    FirmStatus.IN_PROGRESS.value: frozenset(
        {FirmStatus.COMPLETE.value, FirmStatus.NEEDS_REVIEW.value, FirmStatus.IN_PROGRESS.value}
    ),
    FirmStatus.COMPLETE.value: frozenset({FirmStatus.IN_PROGRESS.value}),
    FirmStatus.NEEDS_REVIEW.value: frozenset({FirmStatus.IN_PROGRESS.value}),
}
...
def get_firm(conn: sqlite3.Connection, website: str) -> FirmRecord | None:
    """Read a firm row back out as a FirmRecord, keyed by website. None if missing."""
    row = conn.execute(
...
def insert_extraction(
    ...
    *,
    ...
    """Append one per-field provenance row to the extractions table.

    Append-only (never an upsert): one row per extracted field per firm run.
    Commits the transaction (crash-safe per-row write).
    """
    conn.execute(
        """
...
def enqueue_job(
    conn: sqlite3.Connection, kind: str, payload: str, priority: int = 9
) -> int:
    """Add a job to the queue. Lower priority number = claimed sooner (0 = urgent)."""
    now = datetime.now(timezone.utc).isoformat()
    cur = conn.execute(
        "INSERT INTO jobs (kind, payload, priority, status, attempts, created_at, updated_at) "
        "VALUES (?, ?, ?, 'queued', 0, ?, ?)",
        (kind, payload, priority, now, now),
    )
    conn.commit()
    return int(cur.lastrowid)


def job_already_queued(conn: sqlite3.Connection, kind: str, payload: str) -> bool:
    """True if a queued/in_progress job already exists for this (kind, payload)."""
    row = conn.execute(
        "SELECT 1 FROM jobs WHERE kind = ? AND payload = ? AND status IN ('queued', 'in_progress') LIMIT 1",
        (kind, payload),
    ).fetchone()
...
def claim_next_job(conn: sqlite3.Connection) -> dict | None:
    """Atomically claim the lowest-priority-number, oldest queued job.

    Priority-0 jobs enqueued mid-batch are claimed before any lower-priority
    (higher-number) work already queued, since every claim re-queries the DB —
    no in-memory queue snapshot to go stale.
    """
    now = datetime.now(timezone.utc).isoformat()
    row = conn.execute(
        "SELECT id FROM jobs WHERE status = 'queued' ORDER BY priority ASC, id ASC LIMIT 1"
    ).fetchone()
    if row is None:
        return None
    job_id = row["id"]
...
def finish_job(
    ...
    """Mark a claimed job 'done' or 'failed'. Failure reason logged by the caller
    (worker.py), not stored on the row — payload must stay the stable job target."""
    now = datetime.now(timezone.utc).isoformat()
    conn.execute(
...
def queue_summary(conn: sqlite3.Connection) -> dict[str, int]:
    """Counts of jobs by status, for `pescraper run --summary`."""
    rows = conn.execute("SELECT status, COUNT(*) AS n FROM jobs GROUP BY status").fetchall()
    return {r["status"]: r["n"] for r in rows}


def firm_status_summary(conn: sqlite3.Connection) -> dict[str, int]:
    """Counts of firms by status, for `pescraper run --summary`."""
    rows = conn.execute("SELECT status, COUNT(*) AS n FROM firms GROUP BY status").fetchall()
    return {r["status"]: r["n"] for r in rows}


def all_firms(conn: sqlite3.Connection) -> list[FirmRecord]:
    """All firm rows, for export."""
    rows = conn.execute("SELECT * FROM firms ORDER BY firm_name").fetchall()
    records = []
    for row in rows:
        data = dict(row)
        data["status"] = FirmStatus(data["status"])
        data["needs_review"] = bool(data["needs_review"])
        records.append(FirmRecord(**data))
    return records
