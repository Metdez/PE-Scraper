...
def test_status_lifecycle_walk_and_rejects_disallowed(tmp_path) -> None:
    ...
    try:
        ...
        # complete cannot jump directly back to pending.
        with pytest.raises(ValueError):
            advance_status(conn, "https://acme.example", FirmStatus.PENDING)

        # complete CAN re-enter in_progress (a stale-firm re-check, 90-day re-queue).
        advance_status(conn, "https://acme.example", FirmStatus.IN_PROGRESS)
        assert current() == FirmStatus.IN_PROGRESS.value
        advance_status(conn, "https://acme.example", FirmStatus.NEEDS_REVIEW)
        assert current() == FirmStatus.NEEDS_REVIEW.value

        # needs_review can also re-enter in_progress on a subsequent re-check.
        advance_status(conn, "https://acme.example", FirmStatus.IN_PROGRESS)
        assert current() == FirmStatus.IN_PROGRESS.value
    finally:
        conn.close()
...
# --------------------------------------------------------------------------- #
# 02-02 — get_firm, insert_extraction
# --------------------------------------------------------------------------- #


def test_get_firm_missing_website_returns_none(tmp_path) -> None:
...
def test_get_firm_round_trips_fully_populated_record(tmp_path) -> None:
    ...
    try:
        ...
        upsert_firm(conn, original)
        round_tripped = get_firm(conn, "https://acme.example")
        assert round_tripped == original
    finally:
        conn.close()


def test_get_firm_round_trips_minimal_record(tmp_path) -> None:
    from pescraper.db import connect, get_firm, init_db, upsert_firm
    from pescraper.models import FirmRecord
    ...
    conn = connect(db_path)
    try:
        original = FirmRecord(firm_name="Bare Firm", website="https://bare.example")
        upsert_firm(conn, original)
        round_tripped = get_firm(conn, "https://bare.example")
        assert round_tripped == original
        assert round_tripped.rev_min_musd is None
        assert round_tripped.needs_review is False
    finally:
        conn.close()


def test_insert_extraction_writes_one_row_with_timestamp(tmp_path) -> None:
...
def test_insert_extraction_allows_null_source_and_quote(tmp_path) -> None:
    from pescraper.db import connect, init_db, insert_extraction
    ...
    try:
        ...
        assert row["source_page_url"] is None
        assert row["quote"] is None
    finally:
        conn.close()


def test_insert_extraction_appends_rather_than_upserts(tmp_path) -> None:
    from pescraper.db import connect, init_db, insert_extraction
...
def test_insert_extraction_sql_metacharacters_round_trip_as_data(tmp_path) -> None:
    from pescraper.db import connect, get_firm, init_db, insert_extraction, upsert_firm
    from pescraper.models import FirmRecord
    ...
    try:
        upsert_firm(
            conn, FirmRecord(firm_name="Acme Capital", website="https://acme.example")
        )
        malicious = "'; DROP TABLE firms; --"
        insert_extraction(
            conn,
            firm_website="https://acme.example",
            field="activity",
        ...
        row = conn.execute("SELECT * FROM extractions").fetchone()
        assert row["value"] == malicious
        # The firms table must still exist and be intact — no injection occurred.
        assert get_firm(conn, "https://acme.example") is not None
    finally:
        conn.close()
